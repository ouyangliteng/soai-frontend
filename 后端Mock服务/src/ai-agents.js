...
function generateStudentExplanation(report) {
  const mainProblem = report.problemPoints && report.problemPoints[0] ? report.problemPoints[0] : null;
  const mainRisk = report.riskPoints && report.riskPoints[0] ? report.riskPoints[0] : null;
  const evidence = mainProblem && mainProblem.evidence ? mainProblem.evidence : "本次视频只截取了部分训练片段";
  const focusItems = report.nextTrainingFocus ? report.nextTrainingFocus.slice(0, 2) : [];
  return {
    reportId: report.id,
    title: "本次训练安全复盘",
    explanation: `本次 AI 只依据上传视频中的可见姿态和时间段做辅助复盘。${evidence}，系统将“${mainProblem ? mainProblem.title : "动作稳定性"}”列为优先观察项；${mainRisk ? `“${mainRisk.title}”需要教练结合现场骑乘状态复核。` : "本次未形成独立风险结论。"}。`,
